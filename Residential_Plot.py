...
st.title("\U0001F3E1 Residential Plot Finder by Budget Range")

# Inject custom CSS for styling checkboxes and other elements
st.markdown("""
    <style>
...
if use_budget:
    ...
    with col_b1:
        min_budget = st.number_input("Min Budget (₹)", 0, step=10_000, value=0, key="min_budget")
    with col_b2:
        max_budget = st.number_input("Max Budget (₹)", 1, step=10_000, value=10_000_000, key="max_budget")
...
if use_meter:
    ...
    with col_m1:
        min_meter = st.number_input("Min Area (㎡)", 0.0, step=10.0, value=0.0, key="min_meter")
    with col_m2:
        max_meter = st.number_input("Max Area (㎡)", 1.0, step=10.0, value=5_000.0, key="max_meter")
...
if use_yard:
    ...
    with col_y1:
        min_yard = st.number_input("Min Area (yd²)", 0.0, step=10.0, value=0.0, key="min_yard")
    with col_y2:
        max_yard = st.number_input("Max Area (yd²)", 1.0, step=10.0, value=6_000.0, key="max_yard")
...
if use_feet:
    ...
    with col_f1:
        min_feet = st.number_input("Min Area (ft²)", 0.0, step=10.0, value=0.0, key="min_feet")
    with col_f2:
        max_feet = st.number_input("Max Area (ft²)", 1.0, step=10.0, value=50_000.0, key="max_feet")
...
# SECTION 2: Manual Plot Detail Lookup
st.markdown("---")
